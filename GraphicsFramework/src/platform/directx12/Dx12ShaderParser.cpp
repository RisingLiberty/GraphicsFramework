...
namespace
{
	Dx12ShaderStruct FindStruct(const std::vector<Dx12ShaderStruct>& definedStructs, const std::string& name)
	{
		for (const Dx12ShaderStruct& s : definedStructs)
		{
			if (s.name == name)
				return s;
		}
		//TODO: change this
		return Dx12ShaderStruct("", {});
	}

	UniformDataType Dx12UniformDataTypeToCustomDataType(const std::string& type)
	{
	...
	Dx12ShaderTexture::Type Dx12ShaderTextureTypeStringToEnum(const std::string& type)
	{
		if (type == "Texture2D")		return Dx12ShaderTexture::Type::TEXTURE2D;
		if (type == "TextureCube")		return Dx12ShaderTexture::Type::TEXTURECUBE;

		return Dx12ShaderTexture::Type::UNSPECIFIED;
	}
	...
	Dx12ShaderStruct ParseStruct(const std::string& block)
	{
		...
		while (index < tokens.size())
		{
			...
			UniformDataType data_type = Dx12UniformDataTypeToCustomDataType(type);

			Dx12ShaderUniform field = Dx12ShaderUniform(name, data_type, offset);
			offset += field.size;
			uniforms.push_back(field);
		}
		return Dx12ShaderStruct(struct_name, uniforms);
	}

	Dx12ShaderCBuffer ParseCBuffer(const std::string& block, const std::vector<Dx12ShaderStruct>& definedStructs)
	{
		std::vector<std::string> tokens = Tokenize(block);
		...
		std::unique_ptr<Dx12ShaderCBuffer> buffer;
		...
		while (index < tokens.size())
		{
			...
			if (data_type == UniformDataType::UNSPECIFIED)
			{
				...
				Dx12ShaderStruct s = FindStruct(definedStructs, type);
				assert(!s.uniforms.empty()); // if this asserts, compilation would also fail
				//TODO: fix this
				uniforms.push_back(std::make_unique<Dx12ShaderStruct>(s.name, s.uniforms));
			}
			else
				uniforms.push_back(std::make_unique<Dx12ShaderUniform>(name, data_type));
		}
		return Dx12ShaderCBuffer(buffer_name, reg, uniforms);
	}

	Dx12ShaderTexture ParseTexture(const std::string& statement)
	{
		...
		return Dx12ShaderTexture(name, Dx12ShaderTextureTypeStringToEnum(type), count, reg);
	}

	Dx12ShaderSamplerState ParseSamplerState(const std::string& statement)
	{
		std::vector<std::string> tokens = Tokenize(statement);
		// Support Texture2D and TextureCube for now
		unsigned int index = 0;
		...
		return Dx12ShaderSamplerState(name, count, reg);
	}
...
Dx12ParsedShader::Dx12ParsedShader() = default;
Dx12ParsedShader::~Dx12ParsedShader() = default;

Dx12ShaderParser::Dx12ShaderParser() = default;
Dx12ShaderParser::~Dx12ShaderParser() = default;

Dx12ParsedShader Dx12ShaderParser::Parse(const std::string& code)
{
	Dx12ParsedShader parsed_shader;
