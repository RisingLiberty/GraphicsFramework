...
namespace
{
	D3D12_VERTEX_BUFFER_VIEW GetVertexBufferView(const Dx12VertexBuffer* vb, const VertexLayout* layout)
	{
		...
		vb_view.BufferLocation = vb->GetBuffer()->GetGPUVirtualAddress();
		vb_view.StrideInBytes = layout->GetSize();
		vb_view.SizeInBytes = vb->GetSize();

		return vb_view;
	}
}

Dx12VertexArray::Dx12VertexArray(VertexBuffer* vb, VertexLayout* layout):
